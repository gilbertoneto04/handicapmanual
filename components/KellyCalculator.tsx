...
import { Percent, Hash, GitMerge } from 'lucide-react';

interface KellyCalculatorProps {
  onGoToIncrement?: (data: {
    bankroll: string;
    fairOdd: string;
...
const KellyCalculator: React.FC<KellyCalculatorProps> = ({ onGoToIncrement }) => {
  ...
  useEffect(() => {
    ...
    if (!isNaN(b) && !isNaN(o) && !isNaN(val) && o > 1 && val > 0 && fraction > 0) {
      ...
      if (inputMode === 'prob') {
        ...
        if (val <= 1) { setResult(null); return; }
        probDecimal = 1 / val;
      }
      const netOdds = o - 1;
      ...
      if (adjustedF <= 0) {
        setResult({ stake: '0.00', percentage: '0.00%', color: 'text-red-400' });
      } else {
  ...
  const handleGoToIncrement = () => {
    if (!onGoToIncrement || !result || !odd) return;
    const pct = parseFloat(result.percentage);
    onGoToIncrement({
      bankroll,
      fairOdd: inputMode === 'fairOdd' ? inputValue : '',
      kellyFraction,
      allocations: [{ odd, pct: isNaN(pct) ? '' : pct.toFixed(2) }],
    });
  };

  const canIncrement = !!result && result.color === 'text-emerald-400' && !!odd;
        ...
        <div className="space-y-6">
          ...
          {result && (
            <div className="mt-6 bg-slate-900/50 rounded-xl p-6 border border-slate-700 text-center animate-in slide-in-from-bottom-2">
              ...
              {canIncrement && (
                <button
                  onClick={handleGoToIncrement}
                  className="mt-4 inline-flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/20 transition-colors rounded-lg px-4 py-2 text-sm font-medium"
                >
                  <GitMerge size={15} />
                  Incrementar com nova odd
                </button>
              )}
            </div>
          )}
